...
function stalkerTrace(tid, baseAddr, traceStartAddr, traceEndAddr) {
    Stalker.follow(tid, {
        // events: {
        ...
        transform: (iterator) => {
            ...
            while((instruction = iterator.next()) !== null){
                ...
                const diff = (insAddress - baseAddr).toString(16)
                const isModuleCode = 
                    insAddress.compare(traceStartAddr) >= 0 && insAddress.compare(traceEndAddr) < 0;
                // console.log(diff + " " + instruction + " isModuleCode: " + isModuleCode + " insAddress: " + insAddress + " traceStartAddr: " + traceStartAddr + " traceEndAddr: " + traceEndAddr);
                if (isModuleCode){
                    send({
                        type: 'inst',
                        tid: tid,
...
function stalkerTraceRange(tid, base, size) {
    Stalker.follow(tid, {
        transform: (iterator) => {
            ...
            const isModuleCode = 
                startAddress.compare(base) >= 0 && startAddress.compare(base.add(size)) < 0;
...
function traceAddr(addr, endAddress) {
    ...
    Interceptor.attach(addr, {
        onEnter: function (args) {
            ...
            stalkerTrace(this.tid, targetModule.base, addr, endAddress);
            console.log('trace start addr: ' + addr + ' end ddr: ' + endAddress)
        },
...
(() => {
    ...
    recv("config", (msg) => {
        ...
        if (payload.spawn) {
            ...
            if("endAddress" in payload){
                let endAddress = targetModule.base.add(ptr(payload.endAddress));
                traceAddr(targetAddress, endAddress)
            } else {
                traceAddr(targetAddress, endAddress)
            }
        }
