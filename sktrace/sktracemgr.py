...
class Arm64TraceLog:
    ...
    def on_message(self, payload):
        type = payload['type']
        if type == 'inst':
            # {'type': 'inst', 'tid': 13649, 'block': '0x72a69f6fd4', 'val': '{"address":"0x72a69f6fd4","next":"0x4","size":4,"mnemonic":"bl","opStr":"#0x72a6a36094","operands":[{"type":"imm","value":"492421996692","access":"r"}],"regsAccessed":{"read":[],"written":["lr"]},"regsRead":[],"regsWritten":["lr"],"groups":["call","jump","branch_relative"]}'}
            val = json.loads(payload['val'])  # json ins
            inst = Inst(
                val["address"], payload["block"], val["size"], val["mnemonic"], val["opStr"], payload['moduleBase']
            )
            ...
            # {'type': 'ctx', 'tid': 13649, 'val': '{"pc":"0x72a69f658c","sp":"0x737d4ce460","x0":"0x1","x1":"0x737d4ce120","x2":"0x7387530040","x3":"0x30000","x4":"0x7387530040","x5":"0x300","x6":"0x737d4cddc0","x7":"0x737d4cde58","x8":"0xde4f9c8c0881f194","x9":"0xde4f9c8c0881f194","x10":"0x0","x11":"0x0","x12":"0x0","x13":"0xb066cc71ee1","x14":"0x8e5f943ae0bdd","x15":"0x510af71","x16":"0x72a741a028","x17":"0x72b897a08c","x18":"0x731f60e000","x19":"0x75206453a0","x20":"0x7520645848","x21":"0x74805f2160","x22":"0x737d4ce280","x23":"0x100","x24":"0xc0","x25":"0x2010000","x26":"0x737d4ce120","x27":"0x100","x28":"0x737d4cf000","fp":"0x737d4ce520","lr":"0x71f1df400c"}'}
            ...
            if ctx.pc not in self.inst_dict:
                raise Exception(
                    "No inst addr:{} maybe caused by Interceptor.payload:{}".format(ctx.pc, payload))
...
class Inst:
    ...
    def add_execed_ctx(self, ctx):
        print("{}\t\t\t;sp={}\t\t\t;reg={}".format(
            self.simple_str(), ctx.sp, ctx.general_regs))
        ...
        if changed_regs_arr and len(changed_regs_arr) > 0:
            for reg_val in changed_regs_arr:
                if reg_val.keys[0] in self.changed_regs_dict:
                    self.changed_regs_dict[reg_val.keys[0]].append(
                        reg_val.values[0])
                else:
                    self.changed_regs_dict[reg_val.keys[0]] = (
                        reg_val.values[0])
            ...
            if pre_ctx.sp != ctx.sp:
                if "sp" not in self.changed_regs_dict:
                    ...
                    if print_every_change:
                        eveny_change_str = (
                            "{}\t; sp={}->{}".format(self.simple_str(), pre_ctx.sp, ctx.sp))

            for i in range(31):
                if pre_ctx.general_regs[i] != ctx.general_regs[i]:
                    if "x"+str(i) not in self.changed_regs_dict:
                        self.changed_regs_dict["x"+str(i)] = []
                    self.changed_regs_dict["x" +
                                           str(i)].append(ctx.general_regs[i])
                    if print_every_change:
                        if eveny_change_str:
                            eveny_change_str += (", x{}={}->{}".format(
                                str(i), pre_ctx.general_regs[i], ctx.general_regs[i]))
                        else:
                            eveny_change_str = ("{}\t; x{}={}->{}".format(
                                self.simple_str(), str(i), pre_ctx.general_regs[i], ctx.general_regs[i]))
    ...
    def statistics_changed_regs(self):
        if self.changed_regs_dict:
            ...
            for reg in self.changed_regs_dict:
                change_reg = "\t{}:{}".format(
                    reg, ",".join(self.changed_regs_dict[reg]))
                change_line += change_reg
            print(change_line)

    def simple_str(self):
        cur_addr = hex(int(self.addr, 16) - int(self.moduleBase, 16))
        inst_line = "{}\t{}\t{}".format(cur_addr, self.mnem, self.op)
        # inst_line = "{}\t{}\t{}".format(self.addr, self.mnem, self.op)
        return inst_line

    def __str__(self):
        inst_line = "{}\t{}\t{}\t{}\t{}\t{}".format(
            "instruction", self.addr, self.block, self.size, self.mnem, self.op)
        return inst_line
...
class Arm64Ctx:
    def __init__xxx(self, pc, sp,
        ...
        self.general_regs = [int(x0, 16), int(x1, 16), int(x2, 16), int(x3, 16), int(x4, 16), int(x5, 16), int(x6, 16), int(x7, 16), int(x8, 16), int(x9, 16),
                             int(x10, 16), int(x11, 16), int(x12, 16), int(x13, 16), int(x14, 16), int(
            x15, 16), int(x16, 16), int(x17, 16), int(x18, 16), int(x19, 16),
            int(x20, 16), int(x21, 16), int(x22, 16), int(x23, 16), int(
                x24, 16), int(x25, 16), int(x26, 16), int(x27, 16), int(x28, 16),
            int(fp, 16), int(lr, 16)]
